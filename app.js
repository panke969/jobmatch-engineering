...
async function findJobs(){
  ...
  showNotice(
    sourceFilter==="linkedin"
      ...
      : sourceFilter==="adzuna"
      ? "Searching Adzuna…"
      : "Searching available live job sources, deduplicating and applying your filters…"
  );
  ...
  try{
    ...
    showNotice(
      `Found ${data.total_unique||data.results?.length||0} unique live jobs before local filters. `+
      `${state.searchResults.length} are displayed.${partial}`+
    ...
    showNotice(
      e.name==="AbortError"
        ? "The live search took longer than 18 seconds. The new backend patch should normally return partial results before this point; try again after Vercel redeploy."
