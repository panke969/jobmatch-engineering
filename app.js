...
async function findJobs(){
  ...
  showNotice("Searching multiple engineering role families, removing duplicates and ranking the results…");
  ...
  try{
    ...
    $("apiState").textContent="Live feed connected";
    $("apiState").style.color="#42d469";

    state.searchResults=(data.results||[])
      .map(j=>decorateFeedJob(j))
    ...
    showNotice(
      `Found ${data.total_unique||0} unique jobs from ${data.total_raw||0} raw results. `+
      `${state.searchResults.length} passed your filters${failed?`; ${failed} role searches had provider errors`:""}.`
    );
  }catch(e){
...
function decorateFeedJob(j){
  ...
  let salaryScore=55;
  if(j.salary_max){
    salaryScore=clamp(Math.round((j.salary_max/Math.max(state.prefs.salary||60000,1))*70),40,100);
  }else if(j.salary_min){
    salaryScore=clamp(Math.round((j.salary_min/Math.max(state.prefs.salary||60000,1))*70),40,100);
  }
  ...
  const filteredOut=
    ...
    (state.prefs.english && scored.language==="Dutch" && dutchMandatory(norm(raw))) ||
    (j.salary_max && state.prefs.salary && j.salary_max<state.prefs.salary*.78);
  ...
  $("jobResults").innerHTML=state.searchResults.length?state.searchResults.map((j,i)=>`
    <div class="job-result">
      ...
      <div>
        ...
        <div class="tags">
          ${(j.discovered_by||[]).slice(0,3).map(q=>`<span class="tag">${esc(q)}</span>`).join("")}
    ...
    </div>`).join(""):`<p class="muted">No jobs passed your current filters. Try 14 or 30 days, or lower the salary filter.</p>`;
}
