...
function roleAffinity(title){
  ...
  const rules=[
    [/mechatronic/,96],
    [/integration engineer|system integration/,92],
    [/automation engineer|controls engineer|control engineer/,91],
    [/field service engineer|service engineer/,92],
    [/commissioning engineer/,91],
    [/test engineer|verification engineer|validation engineer/,87],
    [/project engineer/,84],
    [/mechanical engineer|machine engineer/,82],
    [/manufacturing engineer|production engineer|industrialization engineer/,82],
    [/\bnpi\b|new product introduction/,83],
    [/systems engineer|system engineer/,84],
    [/reliability engineer|maintenance engineer/,88],
    [/application engineer/,88]
  ];
  for(const [rx,score] of rules){if(rx.test(t))return score;}
  return /\bengineer\b/.test(t)?72:55;
}
...
function scoreJob(raw,meta={}){
  ...
  let ws=0,wt=0;
  Object.entries(categoryScores).forEach(([k,v])=>{ws+=v*weights[k];wt+=weights[k]});
  const skillScore=Math.round(ws/wt);

  const evidence=buildEvidence(text);
  const evidenceScore=clamp(48+evidence.length*5,48,96);
  const roleScore=roleAffinity(title);

  let overall=Math.round(roleScore*.46 + skillScore*.36 + evidenceScore*.18);

  const language=detectLanguage(text);
  if(dutchMandatory(text)) overall-=26;
  else if(language==="Dutch") overall-=14;

  const ex=excludedHit(text);
  if(ex) overall-=35;

  if(meta.salary_min && state.prefs.salary && meta.salary_min<state.prefs.salary*.80) overall-=5;
  ...
  return {
    overall,categoryScores,projects,high,
    risks:detectRisks(text,meta.salary_min),
    requirements:extractRequirements(raw),
    evidence,language,excluded:ex,
    roleScore,skillScore,evidenceScore
  };
}
...
async function findJobs(){
  ...
  const minMatch=clamp(Number($("searchMinMatch").value)||0,0,100);
  ...
  showNotice("Searching live job sources, deduplicating and applying your filters…");
  ...
  try{
    ...
    state.searchResults=decorated
      ...
      .filter(j=>sourceFilter==="all" || sourceKind(j)===sourceFilter)
      .sort((a,b)=>b.rankScore-a.rankScore);

    renderSearchResults();

    $("filterSummary").textContent=`≥${minMatch}% · ${englishOnly?"English only":"all languages"} · ${sourceFilter==="all"?"all sources":sourceFilter}`;
    showNotice(
      `Provider returned ${data.total_unique||data.results?.length||0} unique jobs. `+
      `${state.searchResults.length} match your current filters.`
    );
  }catch(e){
    $("apiState").textContent="API problem";
    $("apiState").style.color="#ffb42b";
    showNotice(`${e.message}. Check Vercel environment variables and redeploy if needed.`,true);
  }finally{
...
function renderSearchResults(){
    ...
    <div class="job-result">
      ...
      <div>
        ...
        <div class="salary-line ${(!j.salary_min&&!j.salary_max)?"salary-missing":""}">
          ${j.salary_min||j.salary_max?`Salary: ${esc(j.salary)}${j.salaryParsed?" · extracted from description":""}`:"Salary: not disclosed"}
        </div>
  ...
  }).join(""):`<p class="muted">No jobs match the current filters. Lower minimum match, disable English-only, widen max age, or select All sources.</p>`;
}
