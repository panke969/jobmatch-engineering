export default async function handler(req, res) {
  ...
  if ((!appId || !appKey) && !serpKey) {
    return res.status(503).json({
      ...
      setup: "Configure Adzuna keys and/or SERPAPI_KEY in Vercel Environment Variables, then redeploy."
    });
  }
  ...
  const perQuery = Math.max(5, Math.min(20, Number(req.query.per_query || 10)));
  ...
  // Fewer Google Jobs requests to preserve SerpApi quota while still covering the main families.
  const googleQueries = [
    "mechatronics automation engineer",
    "integration systems engineer",
    "project mechanical engineer",
    "service commissioning test engineer"
  ];

  async function fetchAdzuna(q) {
    if (!appId || !appKey) return { provider:"Adzuna", query:q, results:[] };
    ...
    const response = await fetch(url, {
      headers: { "Accept":"application/json", "User-Agent":"JobMatch-Engineering/0.4" }
    });
    const body = await response.json().catch(() => ({}));

    if (!response.ok) {
      return { provider:"Adzuna", query:q, error:body?.exception || body?.error || `Provider returned ${response.status}`, results:[] };
    }
  ...
  async function fetchGoogleJobs(q) {
    if (!serpKey) return { provider:"Google Jobs", query:q, results:[] };
    ...
    if (!response.ok || body.error) {
      return { provider:"Google Jobs", query:q, error:body.error || `Provider returned ${response.status}`, results:[] };
    }

    const now = Date.now();
    const maxAgeMs = age*86400000;

    const results=(body.jobs_results || []).map(job => {
      ...
      const chosen = linkedin || indeed || options[0] || null;
  ...
  try {
    const tasks = [];
    if (appId && appKey) tasks.push(...adzunaQueries.map(fetchAdzuna));
    if (serpKey) tasks.push(...googleQueries.map(fetchGoogleJobs));

    const batches = await Promise.all(tasks);
    const failures = batches.filter(b => b.error).map(b => ({provider:b.provider,query:b.query,error:b.error}));
    ...
    for (const job of merged) {
      // Cross-provider dedupe intentionally ignores provider/id first:
      // the same employer/title/location should appear once.
      ...
      if (!map.has(key)) {
        ...
        // Prefer a direct LinkedIn/Indeed apply link when one source provides it.
    ...
    const results = Array.from(map.values()).sort((a,b)=>{
      const da=a.created?Date.parse(a.created):0, db=b.created?Date.parse(b.created):0;
      return db-da;
    });

    const providers=[];
    if(appId&&appKey)providers.push("Adzuna");
    if(serpKey)providers.push("Google Jobs");
